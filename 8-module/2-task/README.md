...
Как вы помните, в нашем ресторане есть возможность показывать только те товары, которые соответствуют определённым критериям, например, определённой категории товаров, определённа максимальная острота и др. 

Чтобы это работало, нам нужно создать метод `updateFilter(filters)`. В качестве аргумента он принимает объект `filters`:
...
- `filters.noNuts` - `true/false` - показывать только те товары, у которых в свойстве `nuts` такое же значение;
- `filters.vegeterianOnly` - `true/false` - показывать только те товары, у которых в свойстве `vegeterian` такое же значение;
